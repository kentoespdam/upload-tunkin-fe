...
const PreviousButton = memo(({ isDisabled, page }: NavigationButtonProps) => {
...
const NextButton = memo(({ isDisabled, page }: NavigationButtonProps) => {
	const { createURL, navigateToURL } = useURLManager();
...
// Page List Component
const PageList = memo(
	({ page, total_pages }: { page: number; total_pages: number }) => {
		const { createURL, navigateToURL } = useURLManager();
